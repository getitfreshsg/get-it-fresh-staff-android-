...
public class DateUtils {
    public static final String
            DATE_FORMATE_1 = "dd/MM/yyyy",              // 25/12/2016
            ...
            DATE_FORMATE_16 = "dd MMMM yyyy",
            DATE_FORMATE_13 = "dd MMM";
...
//            simpleDateFormat.setTimeZone(TimeZone.getDefault());
    ...
    public static float getDays(String date) { //date in format yyyy-MM-dd
        String[] items = date.split("-");
        for (String item : items) {
            System.out.println("item = " + item);
        }
        Calendar thatDay = Calendar.getInstance();
        thatDay.set(Calendar.DAY_OF_MONTH, Integer.parseInt(items[2]));
        thatDay.set(Calendar.MONTH, Integer.parseInt(items[1]) - 1); // 0-11 so 1 less
        thatDay.set(Calendar.YEAR, Integer.parseInt(items[0]));
